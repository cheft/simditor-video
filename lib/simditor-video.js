Simditor.VideoButton = (function() {
  ...
  VideoButton.i18n = {
    'zh-CN': {
      videoLink: '视频链接',
      description: '支持 mp4、ogg、webm 格式或 &lt;embed&gt; 标签'
    },
    'en-US': {
      videoLink: 'Video Link',
      description: 'Support mp4, ogg, webm format or &lt;embed&gt; tag'
    }
  };
...
Simditor.VideoPopover = class VideoPopover extends Simditor.Popover {
  render() {
    ...
    this.srcEl.on('blur', (e) => {
      ...
      if (val && val.startsWith('<embed')) {
        var $next = $('video').next();
        if ($next[0] && $next[0].tagName === 'EMBED') {
          $next.remove();
        }
        this.$embed = $(val);
        this.$embed.attr({width: 320, height: 240});
